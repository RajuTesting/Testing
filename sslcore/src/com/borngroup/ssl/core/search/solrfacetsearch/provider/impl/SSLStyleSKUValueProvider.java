 ...
 *
 */
public class SSLStyleSKUValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider, Serializable {
    ...
    protected List<String> getDisplayCodeList(final ProductModel product) {
        final List<String> displayCodeList = new ArrayList<String>();
        if (product instanceof ApparelStyleVariantProductModel) {
            displayCodeList.add(product.getCode());
        } else {
            for (final VariantProductModel variant : product.getVariants()) {
                if (variant instanceof ApparelStyleVariantProductModel) {
