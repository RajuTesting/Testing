...
/**
 * SslImprovedSearchColorInAutoSuggestionValueProvider. Index color values of the product which is used in improved search .
 *
 * Created by: osheen.gulati@nagarro.com
 *
 * @author SSL
 */
public class SslImprovedSearchColorInAutoSuggestionValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider,
        Serializable {
    ...
    public Collection<FieldValue> getFieldValues(final IndexConfig indexConfig, final IndexedProperty indexedProperty, final Object model)
        ...
        if (model instanceof ProductModel) {
            final ProductModel apparelProductModel = (ProductModel) model;
            ...
            if (CollectionUtils.isNotEmpty(allCategories)) {
                ...
                for (final CategoryModel cat : allCategories) {
                    if (cat != null && !(cat instanceof ClassificationClassModel)) {
                        ...
                        this.createFieldValueForColorAndGender(parentCategoryForProduct, apparelProductModel, fieldValues, indexedProperty);
                    }
                }
    ...
    /**
     * Creates the field value for Gender which is used in auto suggestion.
     *
     ...
     *
     */
    private void createFieldValueForColorAndGender(final CategoryModel parentCategoryForProduct, final ProductModel product,
                ...
                if (null != pathForParentCategory && pathForParentCategory.size() > 3) {
                    ...
                    if (this.getCategoryLevel() == 3 && pathForParentCategory.get(1).getIsIndexable() != null
                        ...
                        if (product instanceof ApparelStyleVariantProductModel) {
                            if (((ApparelStyleVariantProductModel) product).getColourCode() != null) {
                                this.addFieldValues(fieldValues, indexedProperty, pathForParentCategory.get(3).getName() + "_"
                                        + ((ApparelStyleVariantProductModel) product).getColourCode().toLowerCase());
                            }
                        } else {
                            for (final VariantProductModel variantProductModel : product.getVariants()) {
                                if (((ApparelStyleVariantProductModel) variantProductModel).getColourCode() != null) {
                                    this.addFieldValues(fieldValues, indexedProperty, pathForParentCategory.get(3).getName() + "_"
                                            + ((ApparelStyleVariantProductModel) variantProductModel).getColourCode().toLowerCase());
                                }
                            }
                        }
                    }
