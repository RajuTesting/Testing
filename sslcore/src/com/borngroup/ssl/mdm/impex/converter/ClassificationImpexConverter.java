 ...
 *
 */
package com.borngroup.ssl.mdm.impex.converter;
 ...
 */
public class ClassificationImpexConverter implements ImpexConverter
{
			...
			while (idx > -1)
			{
				...
				else
				{
					...
					if (colValue != null)
					{
						...
						{
							colValue = colValue.replace(";", SEMICOLON_STRING);
						}
	...
	protected String escapeQuotes(final String input)
	{
		...
		{
			if (string.contains(SEMICOLON_STRING))
			{
				string = string.replace(SEMICOLON_STRING, ";");
			}
			if (doesNotContainNewLine(string))
			{
				...
				{
					//appending double quoted for accepting the semicolon in impex
					string = "\"" + string + "\"";
				}
				tmp.add(string);
			}
